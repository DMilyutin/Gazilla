...
import com.gazilla.mihail.gazillaj.views.*
...
class AppDialogs {
    ...
    fun dialogNotification(context: Context, notification: Notification, mainView: MainView, bitmap: Bitmap?) {
        ...
        if (notification.promoId != 0) {
            builder.setPositiveButton("Акции") { _, _ ->
                mainView.openMenuFragmentBeforeNotification("PromoFragment")
            }
        }else{
            when{
    ...
    fun dialogAccessDeleteOldCoctailPromo(context: Context, promoCocktailView: PromoCocktailView, message: String){
        val inflater = LayoutInflater.from(context)
        val dialog = inflater.inflate(R.layout.dialog_error, null)

        val textView = dialog.findViewById<TextView>(R.id.tvErrorDialogError)
        textView.text = message
        ...
        builder.setNegativeButton("Отмена") { _, _ ->
            ...
            promoCocktailView.accessDeleteOldCoctail()
        }
